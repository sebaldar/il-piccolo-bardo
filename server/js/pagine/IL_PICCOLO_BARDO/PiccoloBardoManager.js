...
// Per avere __dirname in un modulo ES 
...
class PiccoloBardoManager {
  ...
  constructor({ uri, user, password, database = 'neo4j' } = {}) {
    this.uri = uri || 'bolt://127.0.0.1:7687';
    ...
    console.log(`******************* Inizializzazione PiccoloBardoManager`);
  ...
  async initNeo4j() {
    try {
      this.driver = await neo4jService.connect(
        this.uri,
        this.user,
        this.password,
        this.database
      );
      console.log(`[Neo4j] Driver pronto per il database: ${this.database}`);
    } catch (err) {
      console.error(`[Neo4j] Errore durante la connessione: ${err.message}`);
      throw err;
    }
  }
...
async generateStoryFromPrompt(userPrompt, age = 5) {
  try {
    ...
    const systemPrompt = `
Sei "Il Piccolo Bardo", narratore di favole per bambini 3-10 anni.
Stile: caldo, rassicurante, poetico, frasi brevi, qualche ripetizione dolce.
Inizia sempre con "C'era una volta...".

Crea una storia NUOVA ispirata ai seguenti frammenti. Non copiare frasi intere, usa temi, immagini, emozioni e tono.
...
REGOLE:
- Protagonista: bambino/a o animale/oggetto parlante
- Massimo 3 personaggi principali
- Piccolo problema gentile, soluzione basata su gentilezza, coraggio semplice o collaborazione
...
- Tono positivo e sereno

LUNGHEZZA:
${age <= 4 ? '200-350 parole' : age <= 6 ? '350-500 parole' : '450-650 parole'}

STRUTTURA:
- Inizio: presentazione protagonista e mondo
...
//    this.generateImageFromStory(generatedStory) ;
...
//fs.writeFileSync("output.mp3", Buffer.from(await response.arrayBuffer()));
  ...
  /**
   * Estrae un prompt visivo strutturato da una storia narrativa
   * @param {string} storyText - testo della storia generata dall'LLM
   * @returns {string} prompt visivo pronto per Image API
   */
  async extractVisualPromptFromStory(storyText) {
      ...
      input: [
        {
          role: "system",
          content: `
  Sei un illustratore professionista di libri per bambini (3–6 anni).
  Il tuo compito è trasformare una storia in UNA singola scena visiva.
  `
        },
        {
          role: "user",
          content: `
  Dalla storia seguente:

  1. Scegli una scena significativa e facilmente illustrabile
  2. Descrivila SOLO in termini visivi
  3. Usa massimo 4 frasi brevi
  4. NON usare dialoghi
  5. NON inserire testo o parole da scrivere nell'immagine
  6. Mantieni uno stile coerente da fiaba illustrata

  STORIA:
  ${storyText}

  Restituisci solo la descrizione visiva.
  `
        }
      ]
    });
    ...
    // Stile fisso consigliato (puoi cambiarlo centralmente)
    const styleSuffix = `
  Illustrazione per libro di fiabe per bambini,
  stile acquerello,
  colori pastello,
  linee morbide,
  atmosfera calda e rassicurante,
  personaggi espressivi e gentili,
  ...
  /**
   * Genera un'immagine a partire da un prompt visivo
   * @param {string} visualPrompt - prompt descrittivo dell'immagine
   * @returns {string} URL pubblico dell'immagine
   */
  async generateImageFromPrompt(visualPrompt) {
    if (!visualPrompt || visualPrompt.length < 20) {
      throw new Error("Prompt visivo non valido o troppo corto");
    }
    ...
    if (!response.data || !response.data[0]?.b64_json) {
      throw new Error("Risposta immagine non valida da OpenAI");
    }
    ...
    // URL pubblico (coerente con Apache)

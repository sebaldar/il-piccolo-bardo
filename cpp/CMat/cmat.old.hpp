...
class Math {
		...
		struct GLOB_VAR {
			...
			// PRECISIONE
			t_type TZERO_ACC = 1e-15;    // per algoritmo TZERO
			t_type RADNEW_ACC = 0.00000001; // per algoritmo ricerca radici Newton
			t_type EVAL_ACC = 0.00000001; // per valutazione sistem
			t_type BCKTRK_ACC = 0.00000001; // per backtraking
			
			// zbrent
			short ITMAX = 100 ;			// 	Maximum allowed number of iterations.
...
//	bool check = false;
	...
	t_type test = 0;
	for (int i = 0; i < n_var; i++) {
		auto temp = fabs(p(i)) / max( fabs(xold(i)), 1.0 );
	...
	)
{
	...
	*/
	...
	t_type stpmax = STPMX * max(POWL(sum, 0.5), n_var);
	...
	)
{
	...
	// una sola variabili
	MessageOut("Verifica se ci sono equazioni che dipendono da una variabile ...");
	size_t old_va = va;
	...
	if ( old_va != va ) {
		...
		if ( non_risolte == 0 ) {
			std::ostringstream oss ( "" );
			oss << "Risolte " << old_va - va << " equazioni!";
			MessageOut( oss.str() );
		}
		else {
			std::ostringstream oss ( "" );
...
*/
	...
	if (fu)	{
		if ( !newBacktracking ( var, sorted_f, va ) ) {
...
bool Math< t_function, t_variable, t_type >::risolveEquazione ( t_function & funz, t_variable & var )
{
	...
	try {
		...
		oss << "===>> Non è stato possibile risolvere la variabile " << var.GetName() << " iterativamente con l'algoritmo di Newton-Raphson.";
		MessageOut( oss.str() );
